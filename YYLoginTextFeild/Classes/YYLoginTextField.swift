...
public enum YYLoginTextFieldType: Int {
    case normal = 0, passWord = 1, verificationCode = 2
}

open class YYLoginTextField: UIView {
    
    // 验证码按钮显示默认样式
    var kVerificationCodeTitleDefault: String = "获取验证码"
    ...
    @IBInspectable public var textFieldType_yy: Int = -1 {
        didSet {
            if (textFieldType_yy == -1) {
                print("error: YYLoginTextField 设置textFieldType_yy")
            }
            ...
            switch YYLoginTextFieldType(rawValue: textFieldType_yy) {
            case .normal?:
                let xW: CGFloat = 30
    ...
    @IBInspectable var iconLeftSpace: CGFloat = 5 {
        didSet {
            icon.frame = CGRect(x: iconLeftSpace, y: self.frame.size.height / 2 - icon.image!.size.height / 2 + iconTop, width: icon.image!.size.width, height: icon.image!.size.height)
        }
    }
    // 输入框距离顶部距离
    ...
    // icon图片名称
    @IBInspectable var iconImageName_yy: String = "" {
        didSet {
    ...
    // 输入框占位内容
    @IBInspectable var placeholderText_yy: String = "请您输入..." {
        didSet {
    ...
    @IBInspectable var verificationCodeTitle_yy: String = "获取验证码" {
        didSet {
            if .verificationCode != YYLoginTextFieldType(rawValue: textFieldType_yy) {
                return
            }
    ...
    // 占位字体颜色
    @IBInspectable var placeholderColor_yy: UIColor = UIColor.white {
        didSet {
    ...
    // 输入内容字体颜色
    @IBInspectable var textFieldColor_yy: UIColor = UIColor.white {
        didSet {
    ...
    lazy var clearButton: UIButton = {
        let v = UIButton()
        v.setImage(getImage(named: "yyLoginTextfield_clear_icon"), for: UIControlState.normal)
    ...
    lazy var eyeButton: UIButton = {
        let v = UIButton()
        v.setImage(getImage(named: "yyLoginTextfield_secret_open_icon"), for: UIControlState.normal)
        v.setImage(getImage(named: "yyLoginTextfield_secret_closed_icon"), for: UIControlState.selected)
    ...
    private func getImage(named: String) -> UIImage {
        return UIImage(named: getImageBundle().path(forResource: named, ofType: "png")!)!
    }
...
extension YYLoginTextField: UITextFieldDelegate {
